App = {
     ...
     displayZombie: function (id, name, dna, level, readyTime, winCount, lossCount) {
          ...
          hash = "#";
          //Background color
          var Bcolor = App.randomHex(dna);
          var Back_ground = '#'+Bcolor;
          //font color
          var Fcolor = '#'+App.invertHex(Bcolor);

          background = hash.concat(Bcolor);
          // Retrieve and fill the zombie template
          var zombieTemplate = $('#zombieTemplate');
          zombieTemplate.find('.panel-heading').attr('style','background-color:'+Back_ground+';color:'+Fcolor)
     ...
     listenToEvents: function () {
          App.contracts.ZombieOwnership.deployed().then(function (instance) {
               ...
               instance.NewLevel({}).watch(function (event) {
                    App.reloadZombies();
               });
          });
     },
     ...
     randomHex: function (dna) {
          var rand_hex = dna.toString(16).slice(0, 6);
          return rand_hex

     },

     invertHex: function (hexnum) {
          var hex2int = parseInt(hexnum,16);
          var invert =  0xffffff ^ hex2int;
          return invert.toString(16)
     },
};
